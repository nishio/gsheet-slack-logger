...
class SlackExtractor:
    ...
    def __init__(
        self, token: str, auto_join: bool = True, skip_channels: List[str] = None
    ):
        """
        初期化
                ...
                for channel in response["channels"]:
                    ...
                    if (
                        ...
                        try:
                            ...
                            print(
                                f"チャンネル {channel['name']} への参加に失敗しました: {e}"
                            )
    ...
    def get_history(
        self, channel_id: str, oldest: str, latest: Optional[str] = None
        ...
        while True:
            try:
                params = {"channel": channel_id, "cursor": cursor, "oldest": oldest}
                if latest:
                ...
                for i in range(len(messages) - 1, -1, -1):
                    ...
                    if msg.get("reply_count"):
                        for reply in self.get_replies(channel_id, msg["ts"]):
                            ...
                            reply["text_readable"] = self._process_text(
                                reply.get("text")
                            )
                            yield reply
    ...
    def extract_to_json(
        ...
        result = {
            "period": {"start": oldest.isoformat(), "end": latest.isoformat()},
            "channels": [],
        }
        ...
        for channel in self.get_channels():
            ...
            if messages:
                channel_file = os.path.join(output_dir, f"{channel_name}.json")
                ...
                print(
                    f"  {len(messages)}件のメッセージを {channel_file} に保存しました"
                )
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Slackの履歴をJSONファイルに抽出するツール"
    )
    parser.add_argument(
        "--token", help="Slack APIトークン", default=os.environ.get("SLACK_TOKEN")
    )
    parser.add_argument("--output-dir", help="出力ディレクトリ", default="./slack_data")
    parser.add_argument(
        "--year", type=int, help="抽出する年（指定しない場合は現在の2ヶ月前）"
    )
    parser.add_argument(
        "--month", type=int, help="抽出する月（指定しない場合は現在の2ヶ月前）"
    )
    parser.add_argument(
        "--last-days",
        type=int,
        help="過去何日分を取得するか（指定した場合はyear, monthは無視）",
    )
    parser.add_argument(
        "--auto-join",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-channels", help="スキップするチャンネルIDのカンマ区切りリスト"
    )
    ...
    if not args.token:
        print(
            "エラー: Slack APIトークンが指定されていません。--tokenオプションまたはSLACK_TOKEN環境変数で指定してください。"
        )
        return 1
    ...
    extractor = SlackExtractor(
        token=args.token, auto_join=args.auto_join, skip_channels=skip_channels
    )
